...
def get_args() -> argparse.Namespace:
    ...
    font_input = parser.add_mutually_exclusive_group(required=True)
    font_input.add_argument("--font-dir", help="Directory containing target fonts.")
    font_input.add_argument("--font-file", help="Single target font file.")
    ...
    parser.add_argument("--auto-split", action="store_true",
                        help="Auto split train/test by ratio (ignore --train-chars-per-font and --test-chars-per-font).")
    parser.add_argument("--train-ratio", type=float, default=0.8,
                        help="Train ratio for auto-split mode (default: 0.8).")
...
def main() -> None:
    ...
    if args.font_file:
        font_dir = Path(args.font_file).parent
        font_files: Optional[List[Path]] = [Path(args.font_file)]
    else:
        font_dir = Path(args.font_dir)
        font_files = None

    if args.auto_split:
        print(f"=== Auto-split mode: train_ratio={args.train_ratio} ===")
        train_chars = None
        test_chars = None
    else:
        train_chars = args.train_chars_per_font
        test_chars = args.test_chars_per_font
